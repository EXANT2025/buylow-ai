...
import { ArrowLeft, ArrowRight, TrendingUp, Shield, Zap, Bot, Target, Clock } from "lucide-react"
...
// Only converts explicit \n in text to <br> for manual line breaks
// No automatic period or comma line breaks
function formatParagraphText(text: string): React.ReactNode {
  // Split by explicit \n for manual line breaks only
  const lines = text.split('\n')

  return lines.map((line, lineIdx) => (
    <span key={lineIdx}>
      {line}
      {/* Add line break between explicit \n lines (except for last line) */}
...
// Single image component - shows full original image without cropping
function ProofImage({
  src,
  alt,
  caption
...
// Two column image layout - both images same height, no cropping, like Naver blog auto-align
function TwoColumnImages({
  ...
  return (
    <div className="my-8 md:my-12">
      {/* Two column grid with aligned heights */}
...
export default function ProofPage() {
  ...
  useEffect(() => {
    ...
    // Track proof page view for default path
    const trackProofView = async () => {
      try {
        await supabase.rpc("increment_buylow_proof_view", { mid: PROOF_DEFAULT_ID })
      } catch (error) {
  ...
  // Handle CTA click - track and navigate
  const handleCtaClick = async (e: React.MouseEvent) => {
    e.preventDefault()
    try {
      await supabase.rpc("increment_buylow_cta", { mid: PROOF_DEFAULT_ID })
    } catch (error) {
      console.error("Failed to track CTA click:", error)
    }
    router.push("/?src=proof")
  }
      ...
      <article className="pt-24 pb-20 px-4 md:px-6">
        <div className="max-w-3xl mx-auto">
          ...
          <section>
            ...
            {/* Auto-playing chart videos - QQQ and XAU */}
          ...
          <section className="text-center">
            <div className="bg-gradient-to-b from-[#0088cc]/10 to-transparent border border-[#0088cc]/20 p-8 md:p-12">
              ...
              <a
                href={DEFAULT_LINKS["landing.telegram.group"]}
                target="_blank"
          ...
          <section>
            ...
            {/* Image 14: Nasdaq 100 (QQQ) Trading Results */}
            <ProofImage
            ...
            {/* Image 13: S&P 500 (SPY) Trading Results */}
            <ProofImage
