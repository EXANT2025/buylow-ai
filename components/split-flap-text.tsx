...
/* ── Singleton Audio — reusable from any component ── */
let _singletonCtx: AudioContext | null = null
let _singletonMuted = false

function getSingletonAudioContext(): AudioContext | null {
  if (typeof window === "undefined") return null
  if (!_singletonCtx) {
    const Ctor = window.AudioContext || (window as any).webkitAudioContext
    if (Ctor) _singletonCtx = new Ctor()
  }
  return _singletonCtx
}

/**
 * Module-level click sound — same oscillator logic as the SplitFlap provider.
 * Can be called from anywhere without React context.
 */
export function playTypingClick() {
  if (_singletonMuted) return
  try {
    const ctx = getSingletonAudioContext()
    if (!ctx) return
...
/** Let external code respect the global mute toggle. */
export function setSingletonMuted(muted: boolean) {
  _singletonMuted = muted
}
...
function SplitFlapChar({ char, index, animationKey, skipEntrance, speed, playClick }: SplitFlapCharProps) {
  ...
  const bgColor = isSettled ? "rgba(0, 0, 0, 0.5)" : "rgba(249, 115, 22, 0.2)"
  ...
  if (isSpace) {
    return (
      <div
        style={{
          width: "0.3em",
          fontSize: "clamp(3.1rem, 12.3vw, 14rem)",
        }}
      />
    )
  }
